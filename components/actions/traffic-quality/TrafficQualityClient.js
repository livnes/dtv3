...
export default function TrafficQualityClient({ initialData, userId }) {
    ...
    // Fetch data when propertyId or dateRange changes
    useEffect(() => {
        if (propertyId) {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.data?.length > 0 && !propertyId) {
                    ...
                    const extractedId = firstProperty.name.split('/').pop();
        ...
        <div className="action-container action-rtl">
            ...
            <div className="action-controls">
                ...
                <button
                    ...
                    className={loading || !propertyId ? 'action-btn-disabled' : 'action-btn-primary'}
                >
            ...
            {data && !loading && (
                <>
                    ...
                    <InsightsSection insights={data.insights} getStatusClass={getStatusClass} />
                    <RecommendationsSection recommendations={data.recommendations} getPriorityClass={getPriorityClass} />
                </>
            )}
        </div>
    );
...
function InsightsSection({ insights, getStatusClass }) {
    if (!insights) return null;

    return (
        <div className="action-insights">
            <h3 className="action-insights-title">💡 תובנות מהנתונים</h3>
...
function RecommendationsSection({ recommendations, getPriorityClass }) {
    if (!recommendations) return null;

    return (
        <div className="action-recommendations">
            <h3 className="action-recommendations-title">🎯 המלצות לפעולה</h3>
